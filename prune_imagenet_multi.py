        ...
        if layer == 4 or layer == 5 or layer == 6 or layer == 7:
            for sublayer, (name, submodule) in enumerate(modules._modules.items()):
                ...
                if isinstance(old_block, NewBasicblock):
                    ...
                    conv3_old_weights = Variable(torch.from_numpy(conv3_old_weights)).cuda()
                    ...
                    conv3_new_weights = torch.mul(conv3_old_weights.double(), conv3_prune_mask, out=None)

                    old_block.conv1.weight = torch.nn.Parameter(conv1_new_weights)
                    old_block.conv2.weight = torch.nn.Parameter(conv2_new_weights)
                    old_block.conv3.weight = torch.nn.Parameter(conv3_new_weights)
                    ...
                    old_block.conv3.weight.data = old_block.conv3.weight.type(torch.FloatTensor)
                    old_block.conv3.weight.data = old_block.conv3.weight.data.cuda()
