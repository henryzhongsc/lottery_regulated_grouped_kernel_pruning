...
def Spectral_Clustering(X, K=8, adj=True, metric='euclidean', sim_graph='fully_connect', sigma=1, knn=10, epsi=0.5, normalized=1):
    ...
    # kmeans, score = get_permutation(V_K, 8, V_K.shape[0])
...
def main():
    ...
    moon_data, moon_labels = make_moons(8, noise=0.05)
    # Compute the adjacency matrix
    Adj_mat = squareform(pdist(moon_data, metric='euclidean', p=2))
    # Spectral clustering...
    spec_re1 = Spectral_Clustering(Adj_mat, K=8, sim_graph='fully_connect', sigma=0.01, normalized=1)
